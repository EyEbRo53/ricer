...
**Storage:** All persistent data is stored in a single SQLite database at `~/.config/ricer/ricer.db`. Session Handler initialises the database connection on first run and passes it to each utility that needs persistence. SQLite gives us ACID transactions, a single file to back up, and proper querying — without the fragility of scattered JSON files and directories.
...
2. Execute the script        → runs the script named in change["script"] (e.g. set_cursor_size)
                               with change["parameters"] as arguments.
                               The script itself calls kde_config_orchestrator internally.
                               Order Manager does not know or care about the orchestrator.
...
- **On session start:** Takes a full snapshot of all relevant KDE config keys (by calling each script's corresponding `get` resource). This is the baseline. Stored in the `snapshots` table.
...
- Templates are stored in the `templates` and `template_changes` tables in the SQLite database.
...
| `session_id` | `s9x8y7`                             |
| `script`     | `set_cursor_size`                    |
| `parameters` | `{"size": 48}` (stored as JSON text) |
| `error_type` | `verification_failed`                |
| `detail`     | `Expected cursor size 48, got 24`    |

**Storage:** `failures` table in `~/.config/ricer/ricer.db`.
...
db = sqlite3.connect("~/.config/ricer/ricer.db")

state_mgr       = StateManager(db, session_id)
config_checker  = ConfigChecker()
template_mgr    = TemplateManager(db, session_id)
failure_handler = FailureHandler(db, session_id)
...
All persistent data lives in a single file: `~/.config/ricer/ricer.db`

```sql
-- One row per MCP session
...
-- Full baseline snapshot taken at session start
CREATE TABLE snapshots (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id   TEXT NOT NULL REFERENCES sessions(session_id),
    config_key   TEXT NOT NULL,        -- e.g. "kcminputrc/Mouse/cursorSize"
    value        TEXT NOT NULL,
    captured_at  TEXT NOT NULL
);

-- Per-change deltas (undo/redo stacks are built from these in memory)
CREATE TABLE deltas (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id   TEXT NOT NULL REFERENCES sessions(session_id),
    change_order INTEGER NOT NULL,     -- matches ChangeEntry.order
...
CREATE TABLE templates (
    ...
    name         TEXT,                 -- NULL until user names it
    created_at   TEXT NOT NULL
);

-- Ordered list of changes within a template
CREATE TABLE template_changes (
    ...
    change_type  TEXT NOT NULL         -- "input" or "display"
);
...
CREATE TABLE failures (
    ...
    script       TEXT NOT NULL,
    parameters   TEXT NOT NULL,        -- JSON string
    error_type   TEXT NOT NULL,        -- verification_failed | script_error | retry_exhausted
    detail       TEXT,
    logged_at    TEXT NOT NULL
);
```

Each utility only touches its own table(s). Session Handler owns the `sessions` table, State Manager owns `snapshots` and `deltas`, Template Manager owns `templates` and `template_changes`, Failure Handler owns `failures`. No utility queries another's tables.
